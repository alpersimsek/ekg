...
from train_uc_sinif_baseline import BaselineECGNet, ResNetSEECGNet, set_seed
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Binary iletim yardimci model deney egitimi.")
    ...
    parser.add_argument("--focus-positive-codes", nargs="*", default=[])
    parser.add_argument("--focus-negative-codes", nargs="*", default=[])
    parser.add_argument("--focus-positive-weight", type=float, default=1.0)
    parser.add_argument("--focus-negative-weight", type=float, default=1.0)
...
def build_train_sampler(
    ...
    focus_positive_codes: set[str],
    focus_negative_codes: set[str],
    focus_positive_weight: float,
    focus_negative_weight: float,
    ...
    for row in rows:
        ...
        if label == 1 and codes.intersection(focus_positive_codes):
            weight *= focus_positive_weight
            pos_hits += 1
        if label == 0 and codes.intersection(focus_negative_codes):
            weight *= focus_negative_weight
            neg_hits += 1
        weight = min(weight, max_focus_weight / len(rows))
        weights.append(weight)
    sampler = WeightedRandomSampler(torch.tensor(weights, dtype=torch.double), len(rows), replacement=True)
    return sampler, {
        "focus_positive_hits": pos_hits,
        "focus_negative_hits": neg_hits,
...
def compute_class_weights(rows: list[dict[str, str]], device: torch.device) -> torch.Tensor:
    counts = Counter(1 if row["target_class"] == "iletim" else 0 for row in rows)
    total = len(rows)
    return torch.tensor([total / (2 * counts[0]), total / (2 * counts[1])], dtype=torch.float32, device=device)


def load_partial_weights(model: nn.Module, checkpoint_path: Path, device: torch.device) -> int:
...
def evaluate(model: nn.Module, loader: DataLoader, criterion: nn.Module, device: torch.device) -> EvalResult:
    ...
    return EvalResult(total_loss / max(total_items, 1), accuracy, precision, recall, f1, confusion.tolist())
...
def main() -> None:
    ...
    focus_positive_codes = set(args.focus_positive_codes)
    focus_negative_codes = set(args.focus_negative_codes)
    sampler = None
    sampler_stats = {"mode": "none"}
    if args.sampler == "weighted":
        sampler, sampler_stats = build_train_sampler(
            train_rows,
            focus_positive_codes,
            focus_negative_codes,
            args.focus_positive_weight,
            args.focus_negative_weight,
            args.max_focus_weight,
        )

    train_loader = DataLoader(BinaryECGDataset(train_rows, augment=args.augment), batch_size=args.batch_size, shuffle=sampler is None, sampler=sampler, num_workers=args.num_workers, pin_memory=torch.cuda.is_available(), persistent_workers=args.num_workers > 0)
    val_loader = DataLoader(BinaryECGDataset(val_rows), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=torch.cuda.is_available(), persistent_workers=args.num_workers > 0)
    test_loader = DataLoader(BinaryECGDataset(test_rows), batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=torch.cuda.is_available(), persistent_workers=args.num_workers > 0)

    if args.model == "resnet_se":
        model = ResNetSEECGNet(num_classes=2, dropout=args.dropout).to(device)
    else:
        model = BaselineECGNet(num_classes=2, dropout=args.dropout).to(device)
    matched_weights = load_partial_weights(model, args.base_checkpoint, device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_result.f1 > best_val_f1:
            ...
            best_epoch = epoch
            torch.save({"model_state_dict": model.state_dict(), "epoch": epoch, "args": vars(args), "class_names": CLASS_NAMES}, best_checkpoint)
    ...
    report = {
        ...
        "focus_positive_codes": sorted(focus_positive_codes),
        "focus_negative_codes": sorted(focus_negative_codes),
        "focus_positive_weight": args.focus_positive_weight,
        "focus_negative_weight": args.focus_negative_weight,
        "sampler_stats": sampler_stats,
        "best_epoch": best_epoch,
        "history": history,
        "validation": {"loss": final_val.loss, "accuracy": final_val.accuracy, "precision": final_val.precision, "recall": final_val.recall, "f1": final_val.f1, "confusion_matrix": final_val.confusion_matrix},
        "test": {"loss": final_test.loss, "accuracy": final_test.accuracy, "precision": final_test.precision, "recall": final_test.recall, "f1": final_test.f1, "confusion_matrix": final_test.confusion_matrix},
