...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-5,
        help="Scheduler icin alt ogrenme orani.",
    )
    parser.add_argument("--scheduler-gamma", type=float, default=0.5)
    parser.add_argument("--scheduler-step-size", type=int, default=10)
    parser.add_argument("--scheduler-milestones", type=int, nargs="*", default=[15, 30, 40])
    ...
    parser.add_argument("--warm-restart-t0", type=int, default=10)
    parser.add_argument("--warm-restart-tmult", type=int, default=2)
    parser.add_argument(
        "--save-every-epoch",
        action="store_true",
        help="Gerekirse her epoch sonunda anlik checkpoint yaz.",
    )
    parser.add_argument(
        "--augment",
        choices=["none", "light"],
...
def build_scheduler(
    ...
    if args.scheduler == "onecycle":
        return (
            torch.optim.lr_scheduler.OneCycleLR(
                ...
                anneal_strategy="cos",
                div_factor=25.0,
                final_div_factor=max(args.lr / max(args.min_lr, 1e-12), 1.0),
            ),
            "batch",
        )
    if args.scheduler == "cosine_warm_restarts":
        return (
            torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                ...
                T_mult=args.warm_restart_tmult,
                eta_min=args.min_lr,
            ),
            "epoch",
        )
    ...
    raise ValueError(f"Desteklenmeyen scheduler: {args.scheduler}")
...
def main() -> None:
    ...
    scheduler, scheduler_step_mode = build_scheduler(args, optimizer, len(train_loader))
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss = train_one_epoch(
            ...
            optimizer,
            device,
            batch_scheduler=scheduler if scheduler_step_mode == "batch" else None,
        )
        ...
        if scheduler is not None and scheduler_step_mode == "epoch":
            scheduler.step()
        elif scheduler is not None and scheduler_step_mode == "plateau":
    ...
    report = {
        ...
        "warm_restart_t0": args.warm_restart_t0,
        "warm_restart_tmult": args.warm_restart_tmult,
